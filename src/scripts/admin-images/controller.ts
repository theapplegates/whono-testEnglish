...
export const initAdminImagesConsole = () => {
  ...
  if (!bootstrap) {
    ...
    statusEl.textContent = 'Image library initialization failed';
    statusLiveEl.textContent = 'Image library initialization failed';
    return;
  }
  ...
  const loadDetailMeta = async (
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Image info read failed';
  ...
  const applyResolvedLocalState = async ({
    ...
    if (shouldPrefetch) {
      busy = true;
      syncControls();
      setStatus('loading', 'Reading the current page of images...', false);
  ...
  const applyBrowseState = async ({ updateLocation }: { updateLocation: boolean }) => {
    ...
    await applyResolvedLocalState({
      ...
      updateLocation,
      successMessage: browsePage.totalCount > 0 ? `Matched ${browsePage.totalCount} images` : 'No images matched the criteria'
    });
  };
  ...
  const loadList = async ({ updateLocation }: { updateLocation: boolean }) => {
    ...
    try {
      ...
      setStatus('ok', result.totalCount > 0 ? `Loaded ${result.totalCount} images` : 'No images matched the criteria', false);
    } catch (error) {
      if (token !== requestToken) return;
      clearLoadFailure(error instanceof Error ? error.message : 'Image list read failed');
    } finally {
  ...
  detailEl.addEventListener('click', async (event) => {
    ...
    if (!copyValue) {
      setStatus('error', `${copyLabel} is empty; cannot copy`);
      return;
    }
  ...
  if (hasLocalBrowse) {
    ...
    if (bootstrap.didRefresh) {
      ...
      setStatus(
        'ok',
        resetToDefaultView
          ? (currentTotalCount > 0 ? `Image library refreshed; all assets returned (${currentTotalCount} images)` : 'Image library refreshed; all assets returned')
          : (currentTotalCount > 0 ? `Image library refreshed (${currentTotalCount} images)` : 'Image library refreshed')
      );
    }
  } else {
    syncControls();
