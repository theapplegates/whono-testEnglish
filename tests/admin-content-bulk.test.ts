...
describe('admin content bulk api', () => {
  ...
  it('bulk-exports source files into a zip report without parsing frontmatter', async () => {
    ...
    expect(report).toContain('# Admin Content bulk download report');
    expect(report).toContain('- Requested: 3');
    expect(report).toContain('- Succeeded: 2');
    expect(report).toContain('- Failed: 1');
    expect(report).toContain('## Failed');
    expect(report).toContain('bits/stale');
    expect(report).toContain('- Status: Failed');
    expect(report).toContain('Error codes: relative_path_mismatch');
